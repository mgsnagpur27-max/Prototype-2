...
import { AlertTriangle, Check, ArrowRight, X, FileWarning } from "lucide-react";

export function EditorPanel() {
  const { openTabs, activeTabId, updateTabContent } = useEditorStore();
  const { 
    autoSaveEnabled, 
    autoSaveDelay, 
    conflicts, 
  ...
  const handleAutoSave = useCallback(
    async (content: string) => {
      ...
      const timer = setTimeout(async () => {
        try {
          ...
          const normalizedPath = activeTab.filePath.startsWith("/")
            ? activeTab.filePath.slice(1)
            : activeTab.filePath;
          await webContainerManager.writeFile(normalizedPath, content);
          
          const hash = content.length.toString(16);
          const now = Date.now();
          setFileMetadata(activeTab.filePath, {
            path: activeTab.filePath,
            hash,
            lastModified: now,
            lastSynced: now,
          ...
          removePendingChange(activeTab.filePath);
          setSyncStatus("idle");
        } catch (error) {
    ...
    [activeTab, autoSaveEnabled, autoSaveDelay, debounceTimer, addPendingChange, removePendingChange, setSyncStatus, setFileMetadata]
  );
  ...
  const handleResolveConflict = async (resolution: "keep_local" | "use_remote") => {
    if (!currentConflict) return;
    
    setResolving(true);
    const conflict = resolveConflict(currentConflict.path, resolution);
    
    if (conflict) {
      const contentToUse = resolution === "keep_local" 
        ? conflict.localContent 
        : conflict.remoteContent;
      
      if (resolution === "use_remote") {
        const tab = openTabs.find(t => t.filePath === conflict.path);
        if (tab) {
      ...
      try {
        const normalizedPath = conflict.path.startsWith("/")
          ? conflict.path.slice(1)
          : conflict.path;
        await webContainerManager.writeFile(normalizedPath, contentToUse);
      } catch (error) {
            ...
            >
              <div className="flex items-center justify-between border-b border-amber-500/20 bg-amber-500/10 px-5 py-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-amber-200/70">External changes detected</p>
                  </div>
                </div>
                <button
              ...
              <div className="p-5 space-y-4">
                ...
                <p className="text-sm text-zinc-400">
                  The file was modified externally while you were editing. Choose how to resolve this conflict:
                </p>
